...
#Tmaze <- read.xls(file.choose(), header = TRUE, sep = ";", quote = "\"",dec = "," )     # another option is read.delim when it is in xls
...
boxplot(Tmaze$PI ~ Tmaze$Fly.line, ylab = "PI", las =2 , ylim = c(-1,1),col= as.character(idGroup$Colour), cex.axis =1.5, cex.lab = 1.2 , par(mar = c(15, 8, 1, 5) + 0.1)) + segments(x0 = 0, y0 = 0, x1 = 30, y1 = 0, col = "blue", lwd = 1)
